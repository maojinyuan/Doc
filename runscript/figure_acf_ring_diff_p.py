...
path = [os.path.join(cf, _) for _ in all_items if os.path.isdir(os.path.join(cf, _)) and (_.startswith("chain"))]
path = path[::-1]
last_str = path.pop()
path.insert(0, last_str)

# p_mode = int(input("input pMode(>=1, default 1): ") or 1) # should >= 1
# pmodes = np.array([1, 2, 3, 4, 6, 8, 10, 25])
pmodes = np.array([1, 2, 5])
fig, ax = plt.subplots()
for i in path:
    for p_mode in pmodes:
        ...
        os.chdir(i)
        # get path/*.dat
        dat_file = np.asarray([file for file in os.listdir() if file.startswith("acf") and file.endswith("ring.dat")])
    ...
    plt.xscale('log')
    plt.title(rf"{os.path.basename(i)}_ring_(p={pmodes})")
    # figure_show()
    plt.savefig(f"{i}_ring_acf_p{pmodes}.png", dpi=600, bbox_inches='tight')
    plt.show()
