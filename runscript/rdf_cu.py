...
if __name__ == '__main__':
    ...
    dcdfile = DCDReader(sys.argv[1])
    box_L = dcdfile._unitcell[0]
    binsize = 0.05
    nframe = 500
    maxbin = int(box_L / 5 / binsize)
    #only chain particle, no rings
    traj = np.array([_.copy() for _ in dcdfile], dtype=np.float32)[-nframe:].reshape(nframe, -1, chain_ring, 3)
    traj = traj[:, :, :chain_N].reshape(nframe, -1, 3)
    rr = cu_rdf(traj, np.array([box_L, box_L, box_L], dtype=np.float32), binsize, maxbin)
    r = np.arange(maxbin) * binsize
    np.save("rdf.npy", np.c_[r, 2 * rr])
    plt.plot(r, 2*rr)
    plt.show()
