...
cf, ff, dcdfile, galafile, datfile = get_current_folder_info()
...
for i in path:
    ...
    os.chdir(i)
    # get path/*.dat
    dat_file = np.asarray([file for file in os.listdir() if file.startswith("acf") and file.endswith("chain.dat")])
...
plt.xscale('log')
plt.title(rf"{cf.split('/')[-1]} (pmode={p_mode})", fontdict={'size':fontsize})
